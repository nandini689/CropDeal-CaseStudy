...
namespace CropDealWebAPI.Repository
{
    public class RegisterRepo : IRegisterRepository<CreateUserDto, UserProfile>
    {
        ...
        public bool[] UserExists(CreateUserDto item)
        {
            ...
            try
            {
                if ((_context.UserProfiles?.Any(e => e.UserEmail == item.UserEmail)).GetValueOrDefault())
                {
                    arr[0]= true;
                }
                if ((_context.UserProfiles?.Any(e => e.UserAccnumber == item.UserAccnumber)).GetValueOrDefault())
                {
                    arr[1] = true;
                }
                 return arr;
